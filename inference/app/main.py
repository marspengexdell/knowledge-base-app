...
class ModelManager:
    ...
    def __init__(self):
        # 使用 self.initialized 标志确保 __init__ 内容只执行一次
    ...
    def _get_chat_format(self, model_path):
        try:
            # 仅加载元数据以获取架构信息，避免完整加载模型
            temp_llama = Llama(model_path=model_path, n_ctx=512, n_threads=1, verbose=False, n_gpu_layers=0)
    ...
    def switch_model(self, new_model_name):
        with self.lock:
            if self.model_name == new_model_name and self.model is not None:
                logger.info(f"模型 '{new_model_name}' 已加载。")
...
class InferenceService(inference_pb2_grpc.InferenceServiceServicer):
    def GetModels(self, request, context):
        ...
        logger.info(f"获取模型列表: {models}, 当前模型: {current_model}")
...
def serve():
    ...
    try:
        ...
        if available_models:
            ...
            logger.warning(f"在 {MODELS_PATH} 目录下未找到任何模型文件。服务将无模型启动。")
    except FileNotFoundError:
