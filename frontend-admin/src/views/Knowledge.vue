...
const fetchDocList = async () => {
  try {
    const { data } = await axios.get('/api/admin/knowledge/list');
    docList.value = (data.docs || []).map(name => ({ name }));
  } catch (err) {
...
const handleDownload = (fileName) => {
  window.open(`/api/admin/knowledge/download?file=${encodeURIComponent(fileName)}`);
};
...
const handleDelete = (fileName) => {
  ElMessageBox.confirm(
    ...
    try {
      await axios.delete('/api/admin/knowledge/delete', { params: { file: fileName } });
